...
(function() {
    ...
    const DashboardView = ({ students, history, config, setConfig, updatePoints, handleUndo }) => {
        ...
        const sortedZizai = [...studentsWithDefaults].sort((a, b) => (b.zizai || 0) - (a.zizai || 0)).slice(0, 5); // Show top 5
        ...
        const dormStats = useMemo(() => {
            ...
            dorms.forEach(d => {
                stats[d.id] = { name: d.name, bonus: 0, discipline: 0, hygiene: 0 };
            });

            const startTime = new Date(dormChartRange.start).getTime();
            const endTime = new Date(dormChartRange.end).getTime() + 86399999; // Include the whole end day

            history.forEach(h => {
                if (normalizePointScene(h.scene) !== "宿舍") return;
                if (h.ts < startTime || h.ts > endTime) return;

                const student = students.find(s => s.id === h.studentId);
                if (!student || !student.dorm || !stats[student.dorm]) return;

                const val = Number(h.val) || 0;
                if (val > 0) {
                    stats[student.dorm].bonus += val;
                } else if (val < 0) {
                    const cat = normalizePointCategory(h.category);
                    if (cat === "纪律") {
        ...
        const rawRecent = history.filter(h => (new Date() - new Date(h.ts)) < 48 * 3600000);
        ...
        rawRecent.forEach(h => {
            if (h.reason === "每日工资") {
                const dateKey = getDateString(new Date(h.ts));
                if (!wageMap[dateKey]) {
                    const aggItem = {
        ...
        const recentHistory = aggregatedHistory.slice(0, 100); // Show up to 100 items
        ...
        const updateScheduleNote = (dateStr, val) => {
            setConfig({ ...config, scheduleNotes: { ...scheduleNotes, [dateStr]: val } });
        };
        ...
        const countdownList = useMemo(() => {
            ...
            return countdownEvents
                ...
                .map(e => {
                    const target = getStartOfDay(new Date(e.date));
        ...
        const penaltyLastMap = useMemo(() => {
            ...
            (history || []).forEach(h => {
                ...
                const prev = map.get(h.studentId) || 0;
                if (h.ts > prev) map.set(h.studentId, h.ts);
            });
            return map;
        }, [history]);
        const getNoPenaltyText = (s) => {
            const lastFromHistory = penaltyLastMap.get(s.id) || 0;
        ...
        const handleDutyChange = (day, idx, name) => {
            const newDuty = { ...config.duty }; newDuty[day][idx] = name; setConfig({ ...config, duty: newDuty });
        };
        const handleCommissionerChange = (roleId, studentId) => {
            setConfig({ ...config, commissioners: { ...config.commissioners, [roleId]: parseInt(studentId) } });
        };
        ...
        return h("div", { className: "space-y-6 animate-fade-in" },
            h("div", { className: "bg-white p-4 rounded-xl shadow-sm flex flex-wrap gap-3 items-center" },
                ...
                h("div", { className: "flex flex-wrap gap-2" },
                    countdownList.map(e => h("div", { key: e.id || `${e.name}-${e.date}`, className: "px-3 py-1 rounded-full text-xs bg-blue-50 text-blue-700 border border-blue-100" }, `${e.name} ${e.diff >= 0 ? `还有${e.diff}天` : `已过${Math.abs(e.diff)}天`}`))
                )
            ),
                ...
                h("div", { className: "space-y-6" },
                    h("div", { className: "bg-white p-4 rounded-xl shadow-sm" },
                        ...
                        h("div", { className: "space-y-2" },
                            scheduleDays.map(d => h("div", { key: d.date, className: "flex items-start gap-2" },
                                h("div", { className: "w-28 text-xs text-gray-500 pt-2" }, d.label),
                                h("textarea", { className: "flex-1 border rounded p-2 text-sm bg-gray-50", rows: 2, value: scheduleNotes[d.date] || "", onChange: e => updateScheduleNote(d.date, e.target.value), placeholder: "添加日程" })
                            ))
                        )
                    ),
                    h("div", { className: "bg-white p-4 rounded-xl shadow-sm" },
                ...
                h("div", { className: "space-y-6" },
                    h("div", { className: "bg-white p-4 rounded-xl shadow-sm flex flex-col" },
                        ...
                        h("div", { className: "flex-1 space-y-4" },
                            sortedZizai.length === 0 ? h("div", { className: "text-center text-gray-400 py-4" }, "暂无数据") : sortedZizai.map((s, idx) => {
                                const isTop3 = idx < 3;
                                ...
                                return h("div", { key: s.id, className: `flex items-center gap-4 p-2 rounded-lg transition border shimmer-effect ${isTop3 ? `bg-gradient-to-r ${cardGradients[idx]} animate-gradient-x border-white shadow-md` : 'border-transparent hover:bg-orange-50'}` },
                                    ...
                                    h("div", { className: "font-mono font-bold text-orange-600 text-lg" }, s.zizai ?? 0)
                                );
                            })
                        )
                    ),
                    h("div", { className: "bg-white p-4 rounded-xl shadow-sm" },
                        ...
                        h("div", { className: "space-y-6 pt-2" },
                            ...
                            (() => {
                                const maxVal = Math.max(...dormStats.map(d => Math.max(d.bonus, d.discipline, d.hygiene)), 10);
                                return dormStats.map(d => h("div", { key: d.name, className: "space-y-1" },
                                    h("div", { className: "text-xs font-bold text-gray-600 flex justify-between" }, h("span", null, d.name)),
                                    h("div", { className: "space-y-1.5" },
                                        // Bonus Bar
                                        h("div", { className: "flex items-center gap-2" },
                                            h("div", { className: "flex-1 h-3 bg-gray-100 rounded-full overflow-hidden" },
                                                h("div", { className: "h-full bg-green-500 transition-all", style: { width: `${(d.bonus / maxVal) * 100}%` } })
                                            ),
                                            h("span", { className: "text-[10px] w-8 font-mono text-green-600" }, `+${d.bonus}`)
                                        ),
                                        // Discipline Bar
                                        h("div", { className: "flex items-center gap-2" },
                                            h("div", { className: "flex-1 h-3 bg-gray-100 rounded-full overflow-hidden" },
                                                h("div", { className: "h-full bg-red-400 transition-all", style: { width: `${(d.discipline / maxVal) * 100}%` } })
                                            ),
                                            h("span", { className: "text-[10px] w-8 font-mono text-red-500" }, `-${d.discipline}`)
                                        ),
                                        // Hygiene Bar
                                        h("div", { className: "flex items-center gap-2" },
                                            h("div", { className: "flex-1 h-3 bg-gray-100 rounded-full overflow-hidden" },
                                                h("div", { className: "h-full bg-yellow-400 transition-all", style: { width: `${(d.hygiene / maxVal) * 100}%` } })
                                            ),
                                            h("span", { className: "text-[10px] w-8 font-mono text-yellow-600" }, `-${d.hygiene}`)
                                        )
                                    )
                                ));
                            })()
                ...
                h("div", { className: "space-y-6" },
                    h("div", { className: "bg-white p-4 rounded-xl shadow-sm" },
                        ...
                        h("div", { className: "space-y-3" },
                            sortedPenalty.map((s, idx) => {
                                const isTop3 = idx < 3;
                                return h("div", { key: s.id, className: "flex items-center gap-3 text-sm relative" },
                                    ...
                                    h("div", { className: "flex-1" },
                                        ...
                                        h("div", { className: "text-xs text-gray-400" }, getNoPenaltyText(s))
                                    ),
                                    h("span", { className: "text-red-500 font-bold" }, s.penalty)
                                );
                            })
                        )
                    ),
                    h("div", { className: "bg-white p-4 rounded-xl shadow-sm" },
                        ...
                        h("div", { className: "space-y-2 max-h-96 overflow-y-auto" },
                            recentHistory.map(hItem => h("div", { key: hItem.id, className: "text-xs flex justify-between p-2 bg-gray-50 rounded" },
                                h("div", null,
                                    h("span", { className: "font-bold mr-2" }, hItem.studentName),
                                    h("span", { className: "text-gray-500" }, hItem.isAggregated ? `${hItem.reason} (${hItem.count}人)` : hItem.reason)
                                ),
                                h("div", { className: "flex items-center gap-2" },
                                    h("span", { className: hItem.val > 0 ? "text-green-600" : "text-red-600" }, hItem.val > 0 ? `+${hItem.val}` : hItem.val),
                                    !hItem.isAggregated && !hItem.isUndoLog && h("button", { onClick: () => handleUndo(hItem.id), className: "text-gray-300 hover:text-blue-500", title: "撤销" }, h(Icon, { name: "undo", size: 12 }))
                                )
                            ))
                        )
                    )
    ...
    const BattleView = ({ students, setStudents, battle, setBattle, batchUpdatePoints, isDirtyRef, isSavingRef, persistData }) => {
        ...
        const calcMemberDetail = (student) => {
            ...
            const base = getBaseRank(student);
            const settle = getSettleRank(student);
        ...
        const buildTeamDetail = (team) => {
            const m1 = (Array.isArray(students) ? students : []).find(s => s.id === team.memberIds?.[0]);
            const m2 = (Array.isArray(students) ? students : []).find(s => s.id === team.memberIds?.[1]);
            const d1 = calcMemberDetail(m1);
            const d2 = calcMemberDetail(m2);
        ...
        const teamRankSum = (team) => {
            ...
            return memberIds.reduce((acc, id) => {
                const s = (Array.isArray(students) ? students : []).find(stu => stu.id === id);
                if (!s) return acc + totalStudents;
                const rank = getBaseRank(s);
        ...
        const updateTeam = (id, patch) => {
            setBattle(prev => {
                ...
                return { ...b, teams: b.teams.map(t => t.id === id ? { ...t, ...patch } : t) };
            });
        };
        ...
        const updateSquad = (id, patch) => {
            setBattle(prev => {
                const b = battleNormalize(prev);
                return { ...b, squads: b.squads.map(s => s.id === id ? { ...s, ...patch } : s) };
            });
        };
        ...
        const updateBattle = (id, patch) => {
            setBattle(prev => {
                const b = battleNormalize(prev);
                return { ...b, battles: b.battles.map(bt => bt.id === id ? { ...bt, ...patch } : bt) };
            });
        };
        ...
        const addTeam = () => {
            setBattle(prev => {
                const b = battleNormalize(prev);
                const idx = b.teams.length + 1;
                return { ...b, teams: [...b.teams, { id: `t${idx}`, name: `双子星第${idx}组`, points: BATTLE_INITIAL_POINTS, memberIds: ['', ''] }] };
            });
        };
        ...
        const addSquad = () => {
            setBattle(prev => {
                const b = battleNormalize(prev);
                const idx = b.squads.length + 1;
                return { ...b, squads: [...b.squads, { id: `sq${idx}`, name: `共鸣${idx}`, teamIds: ['', ''] }] };
            });
        };
        ...
        const removeTeam = (id) => {
            setBattle(prev => {
                const b = battleNormalize(prev);
                return { ...b, teams: b.teams.filter(t => t.id !== id), squads: b.squads.map(sq => ({ ...sq, teamIds: (sq.teamIds || []).map(tid => tid === id ? '' : tid) })), battles: b.battles.filter(bt => bt.teamAId !== id && bt.teamBId !== id) };
            });
        };
        ...
        const removeSquad = (id) => {
            setBattle(prev => {
                const b = battleNormalize(prev);
                return { ...b, squads: b.squads.filter(s => s.id !== id) };
            });
        };
        ...
        const removeBattle = (id) => {
            setBattle(prev => {
                const b = battleNormalize(prev);
                return { ...b, battles: b.battles.filter(bt => bt.id !== id) };
            });
        };

        const handleInit = () => {
            if (!confirm("将按当前学生初始化队伍与共鸣，现有配置会被覆盖，继续吗？")) return;
            const initTeams = battleInitTeams(students);
            const initSquads = battleInitSquads(initTeams);
            setBattlePatch({ teams: initTeams, squads: initSquads, battles: [] });
            setResults(null);
        };
        ...
        const handleResetPoints = () => {
            ...
            setBattle(prev => {
                ...
                return {
                    ...b,
                    teams: b.teams.map(t => ({ ...t, points: BATTLE_INITIAL_POINTS }))
                };
            });
        };
        ...
        const handleAddChallenge = () => {
            ...
            setBattle(prev => {
                const b = battleNormalize(prev);
                return { ...b, battles: [...b.battles, battleItem] };
            });
        ...
        const handleRefuse = (battleId) => {
            ...
            setBattle(prev => {
                const b = battleNormalize(prev);
                const newTeams = b.teams.map(t => {
                    if (t.id !== target.teamBId) return t;
                    return { ...t, points: (Number(t.points) || 0) - stakeVal };
                });
                const msg = `${b.teams.find(t => t.id === target.teamBId)?.name || '应战方'} 拒绝挑战 -${stakeVal}`;
                return { ...b, teams: newTeams, battles: b.battles.filter(bt => bt.id !== battleId), logs: [{ time: getNow().toLocaleTimeString(), msg }, ...b.logs] };
            });
        };
        ...
        const handleAutoMatch = () => {
            ...
            setBattle(prev => {
                const b = battleNormalize(prev);
                return { ...b, battles: [...b.battles, ...newBattles] };
            });
        };

        const simulate = () => {
            if (!teamBaseExamId || !settleExamId) return alert("请先选择组队基准与结算考试");
            const tRes = teams.map(t => {
                const detail = buildTeamDetail(t);
                const currentPts = Number(t.points) || 0;
                return { id: t.id, name: t.name, currentPts, cp: detail.cp, k: detail.k, newPts: currentPts, msgs: [], won: false, isSafe: detail.memberDetails.some(d => d.safe), members: detail.members, memberDetails: detail.memberDetails };
            });

            const pBattles = battles.map(b => {
                const A = tRes.find(x => x.id === b.teamAId);
                const B = tRes.find(x => x.id === b.teamBId);
                if (!A || !B) return { ...b, winId: null, outcomeTag: "无效对战", detail: null };
                ...
                if (winner === 'A') {
                    A.won = true;
                    if (b.isUnderdog) {
                        A.newPts = A.currentPts * 2 + b.stake;
                        B.newPts = B.currentPts - b.stake;
                        A.msgs.push("下克上翻倍");
                        outcomeTag = "下克上成立";
                        formulaA = `当前积分×2 + 赌注(${b.stake})`;
                        formulaB = `当前积分 - 赌注(${b.stake})`;
                    } else {
                        A.newPts = A.currentPts * 2 + b.stake;
                        B.newPts = -20 - b.stake;
                        outcomeTag = "常规胜";
                        formulaA = `当前积分×2 + 赌注(${b.stake})`;
                        formulaB = `-20 - 赌注(${b.stake})`;
                    }
                } else if (winner === 'B') {
                    B.won = true;
                    if (b.isUnderdog) {
                        A.newPts = A.currentPts * 0.5 - b.stake;
                        B.newPts = B.currentPts * 2 + b.stake;
                        A.msgs.push("下克上抚恤");
                        outcomeTag = "下克上失败";
                        formulaA = `当前积分×0.5 - 赌注(${b.stake})`;
                        formulaB = `当前积分×2 + 赌注(${b.stake})`;
                    } else {
                        if (A.isSafe) {
                            A.newPts = A.currentPts * 0.5 - b.stake;
                            B.newPts = B.currentPts * 2 + b.stake;
            ...
            const squadBonuses = squads.map(sq => {
                const t1 = tRes.find(r => r.id === sq.teamIds?.[0]);
                const t2 = tRes.find(r => r.id === sq.teamIds?.[1]);
                if (t1?.won && t2?.won) {
            ...
            const finalBattles = pBattles.map(b => {
                ...
                return {
                    ...b,
                    detail: {
                        teamA: { ...b.detail.teamA, newPts: A.newPts, delta: A.newPts - A.currentPts, msgs: [...A.msgs] },
                        teamB: { ...b.detail.teamB, newPts: B.newPts, delta: B.newPts - B.currentPts, msgs: [...B.msgs] }
                    }
                };
            });
            setResults({ tRes, pBattles: finalBattles, squadBonuses });
        };
...
const updates = [];
        ...
        const startNewSeason = () => {
            ...
            setBattle(prev => {
                const b = battleNormalize(prev);
                return { ...b, teams: b.teams.map(t => ({ ...t, points: BATTLE_INITIAL_POINTS })), battles: [], logs: [], history: [seasonRecord, ...b.history], season: b.season + 1 };
            });
            setResults(null);
            alert("新赛季已开启");
        };
